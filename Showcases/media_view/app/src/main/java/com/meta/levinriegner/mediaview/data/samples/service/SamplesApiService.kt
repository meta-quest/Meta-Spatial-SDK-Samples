...
constructor(
  ...
  suspend fun getSamplesList(): SamplesList {
    val request = Request.Builder().url(FILE_INDEX_URL).build()
  ...
  fun downloadFile(
    ...
    val fileSize =
        httpClient.newCall(sizeRequest).execute().use {
    ...
    for (i in 0 until totalChunks) {
      ...
      val chunkRequest = Request.Builder().url(url).header("Range", range).build()
