  ...
  fun updateParent(assetUuid: Int, type: AssetType, parentUuid: Int) {
    ...
    when (type) {
      AssetType.STICKY_NOTE -> {
        table = STICKIES_TABLE
        _uuid = STICKY_UUID
        values = ContentValues().apply { put(STICKY_PARENT, parentUuid) }
      }
      AssetType.TASK -> {
        table = TASKS_TABLE
        _uuid = TASK_UUID
        values = ContentValues().apply { put(TASK_PARENT, parentUuid) }
      }
      else -> {
        table = TOOLS_TABLE
        _uuid = TOOL_UUID
        values = ContentValues().apply { put(TOOL_PARENT, parentUuid) }
      }
    }
