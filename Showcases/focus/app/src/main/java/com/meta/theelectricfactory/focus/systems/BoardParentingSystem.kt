...
// This system allow us to detect when an object is close to a board and stick to it by setting it
// as its parent.
class BoardParentingSystem : SystemBase() {
  ...
  override fun execute() {
    ...
    // IMPORTANT NOTE: This method below is not working properly right now due to some issues with
    // parenting property in the SDK.
    // Instead of using this, we are imitating the parenting behaviour by making a "stick" effect
    // when objects are close to a board.
    ...
    //            val distance =
    //                pointToRectangleDistance(getAbsoluteTransform(grabbedChild).t, vertices[0],
    ...
    //                grabbedChild.hasComponent<TransformParent>() &&
    ...
    //                    grabbedChild.getComponent<TransformParent>().entity != board) {
    //
    //                    val currentParentVertices =
    // getMeshBoundsAbsolutePosition(grabbedChild.getComponent<TransformParent>().entity)
    //                    val distanceToCurrentParent =
    // pointToRectangleDistance(getAbsoluteTransform(grabbedChild).t, currentParentVertices[0],
    // currentParentVertices[1], currentParentVertices[2], currentParentVertices[3])
    //
    //                    // If the object is still close to its current parent, do not change
    // parent
    ...
    // IMPORTANT NOTE: Since the parenting property isn't working properly in the SDK, we are
    // imitating the parenting behaviour by making a "stick" effect when objects are close to a
    // board.
