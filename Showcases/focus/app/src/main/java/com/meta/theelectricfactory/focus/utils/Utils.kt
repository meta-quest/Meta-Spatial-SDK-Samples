...
fun addOnSelectListener(entity: Entity, onClick: () -> (Unit)) {
  entity.registerEventListener<ButtonDownEventArgs>(ButtonDownEventArgs.EVENT_NAME) { ent, eventArgs
    ->
    if (
...
fun deleteObject(
  ...
  // Check if we have to delete it from database as well or we are just cleaning the space to show a
  // different project
